    ...
    try:
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.warning(f"Qdrant connection attempt {retry_count}/{max_retries} failed: {e}. Retrying in {wait_time}s...")
        ...
        logger.error(f"Failed to connect to Qdrant after {max_retries} retries: {last_exception}")
        raise QdrantConnectionError(f"Failed to connect to Qdrant after {max_retries} retries: {last_exception}")
...
class VectorStore:
    ...
    def store_documents(
        ...
        try:
            ...
            if actual_dim != self.vector_size:
                logger.warning(f"Embedding dimension mismatch. Expected {self.vector_size}, got {actual_dim}. Recreating collection.")
            ...
            for i in range(0, total_docs, batch_size):
                ...
                for doc in batch:
                    ...
                    if not doc_id:
                        doc_id = str(hash(frozenset(doc.items())))
                ...
                logger.info(f"Stored batch of {len(batch)} documents ({i+len(batch)}/{total_docs})")
    ...
    def search(
        ...
        try:
            ...
            search_filter = None
            if filter_condition:
                search_filter = models.Filter(**filter_condition)
    ...
    def delete_by_filter(self, filter_condition: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully deleted documents matching filter from vector store")
            return True
