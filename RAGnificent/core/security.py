...
def redact_sensitive_data(text: str, patterns: Optional[List[Tuple[str, str]]] = None) -> str:
    """
    ...
    default_patterns = [
        (r'(api[_-]?key|token)["\']?\s*[:=]\s*["\']?([a-zA-Z0-9]{20,})["\']?', r'\1: [REDACTED]'),
...
def sanitize_content(content: str) -> str:
    ...
    content = re.sub(r'<(iframe|object|embed).*?>.*?</\1>', '', content, flags=re.IGNORECASE | re.DOTALL)
