...
class RequestThrottler:
        ...
        with self.lock:
            ...
            if url:
                ...
                if stats.backoff_until and current_time < stats.backoff_until:
                    ...
                    logger.debug(f"Domain {domain} in backoff mode, sleeping for {backoff_sleep:.2f}s")
                ...
                if time_since_domain_last < domain_interval:
                    ...
                    logger.debug(f"Throttling request to {domain} for {sleep_time:.2f}s")
    ...
    def release(self, url: Optional[str] = None, status_code: Optional[int] = None, 
                response_time: Optional[float] = None, error: Optional[Exception] = None) -> None:
        """
            ...
            if url and self.enable_stats:
                ...
                if error:
                    ...
                    if stats.consecutive_errors > 1:
                        backoff_time = min(60, self.retry_delay * (2 ** (stats.consecutive_errors - 1)))
        ...
        while retries <= self.max_retries:
            ...
            try:
                ...
                if status_code in (429, 503):
                    ...
                    logger.warning(f"Rate limited by {domain} (status {status_code}), waiting {retry_after}s")
    ...
    def execute_parallel(self, func: Callable, urls: List[str], *args, **kwargs) -> List[Any]:
        """
    ...
    def _get_retry_after(self, response: Any) -> float:
        ...
        if hasattr(response, 'headers') and 'Retry-After' in response.headers:
            ...
            try:
                ...
                try:
                    retry_date = datetime.strptime(retry_value, "%a, %d %b %Y %H:%M:%S %Z")
        ...
        with self.lock:
            ...
            for domain, domain_stats in self.domain_stats.items():
                ...
                if domain_stats.request_times:
                    avg_response_time = sum(domain_stats.request_times) / len(domain_stats.request_times)
