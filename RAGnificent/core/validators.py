...
def validate_chunk_params(chunk_size: int, chunk_overlap: int) -> bool:
    ...
    if chunk_overlap >= chunk_size:
        logger.warning(f"Chunk overlap ({chunk_overlap}) must be less than chunk size ({chunk_size})")
        return False
...
class InputValidator:
    ...
    def validate_input(input_type: str, value: Any) -> bool:
        ...
        validators = {
            ...
            "chunk_params": lambda v: validate_chunk_params(v.get("chunk_size", 0), v.get("chunk_overlap", 0)),
