...
def assert_performance_budget(
    actual: float,
    max_duration: float,
    description: str = "Performance budget"
) -> None:
    """
...
def assert_timing_statistical(
    ...
    assert_timing_within(median_time, expected, tolerance_pct, 
...
class TimingCategories:
    ...
    RATE_LIMIT_TOLERANCE = 12.0  # ±12% (slightly increased to handle system timing variations)
...
def assert_immediate_operation(duration: float, description: str = "Immediate operation") -> None:
    """Assert that an operation completes immediately (< 50ms)."""
    assert_performance_budget(duration, PerformanceBudgets.THROTTLE_IMMEDIATE, description)


def assert_rate_limit_timing(
    actual: float, 
    expected: float, 
    description: str = "Rate limit timing"
) -> None:
    """Assert rate limiting timing with appropriate tolerance."""
    assert_timing_within(actual, expected, TimingCategories.RATE_LIMIT_TOLERANCE, description)


def assert_critical_timing(
    actual: float, 
    expected: float, 
    description: str = "Critical timing"
) -> None:
    """Assert critical timing operations with tight tolerance."""
    assert_timing_within(actual, expected, TimingCategories.CRITICAL_TOLERANCE, description)