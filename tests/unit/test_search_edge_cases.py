...
from RAGnificent.rag.search import SearchResult, SemanticSearch
...
class TestSearchEdgeCases(unittest.TestCase):
    ...
    def test_search_result_from_dict(self):
        ...
        self.assertEqual(
            result.content, "", "Empty dict should create result with empty content"
        )
        self.assertEqual(
            result.score, 0.0, "Empty dict should create result with zero score"
        )
        self.assertEqual(
            result.document_id,
            "",
            "Empty dict should create result with empty document_id",
        )
        ...
        self.assertEqual(
            result.metadata, {}, "Should use empty dict for metadata when missing"
        )
