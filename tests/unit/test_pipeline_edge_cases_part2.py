...
class TestPipelineEdgeCasesPart2(unittest.TestCase):
    ...
    def test_pipeline_step_dependencies(self):
        ...
        with mock.patch("RAGnificent.rag.pipeline.Pipeline.extract_content") as mock_extract:
            ...
            self.assertFalse(result["steps"].get("documents", True), "Documents step should be marked as failed")
            self.assertFalse("chunks" in result["steps"], "Chunks step should not be executed")

    def test_missing_sitemap(self):
        """Test handling of missing sitemap."""
        urls = self.pipeline._get_urls_from_sitemap("https://example.com/nonexistent-sitemap.xml", None)
    ...
    def test_malformed_sitemap(self):
        ...
        with mock.patch("RAGnificent.utils.sitemap_utils.SitemapParser.parse_sitemap") as mock_parse:
            mock_parse.side_effect = Exception("XML parsing error")
            
            urls = self.pipeline._get_urls_from_sitemap("https://example.com/sitemap.xml", None)
    ...
    def test_missing_links_file(self):
        ...
        urls = self.pipeline._get_urls_from_file(str(self.test_files_dir / "nonexistent.txt"), None)
        self.assertEqual(len(urls), 0, "Non-existent links file should return empty list")
    ...
    def test_query_with_context_no_openai(self):
        ...
        with mock.patch.dict("sys.modules", {"openai": None}):
            ...
            self.assertFalse(result["has_context"], "Should indicate no context when OpenAI is not available")
    ...
    def test_query_with_context_no_results(self, mock_search):
        ...
        result = self.pipeline.query_with_context("test query")
        self.assertFalse(result["has_context"], "Should indicate no context when no results found")
        self.assertIn("couldn't find", result["response"].lower(), "Response should indicate no information found")
    ...
    def test_get_pipeline_singleton(self):
        ...
        self.assertIs(pipeline1, pipeline2, "get_pipeline should return the same instance")
        
        pipeline3 = get_pipeline(collection_name="different_collection")
        self.assertIs(pipeline1, pipeline3, "get_pipeline should still return the same instance")
