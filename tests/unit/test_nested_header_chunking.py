...
class TestNestedHeaderChunking(unittest.TestCase):
    ...
    def test_section_parsing(self):
        ...
        self.assertEqual(nested_levels, [0, 1, 2, 1, 2], "Nested levels should match expected hierarchy")
        
        self.assertEqual(len(sections[0]['parent_headers']), 0, "Main topic should have no parent headers")
        self.assertEqual(len(sections[1]['parent_headers']), 1, "Subtopic 1 should have 1 parent header")
        self.assertEqual(len(sections[2]['parent_headers']), 2, "Nested Subtopic 1.1 should have 2 parent headers")
        self.assertEqual(len(sections[3]['parent_headers']), 1, "Subtopic 2 should have 1 parent header")
        self.assertEqual(len(sections[4]['parent_headers']), 2, "Nested Subtopic 2.1 should have 2 parent headers")
        
        self.assertEqual(sections[2]['parent_headers'][0]['text'], "Main Topic", "First parent of Nested Subtopic 1.1 should be Main Topic")
        self.assertEqual(sections[2]['parent_headers'][1]['text'], "Subtopic 1", "Second parent of Nested Subtopic 1.1 should be Subtopic 1")
        self.assertEqual(sections[4]['parent_headers'][0]['text'], "Main Topic", "First parent of Nested Subtopic 2.1 should be Main Topic")
        self.assertEqual(sections[4]['parent_headers'][1]['text'], "Subtopic 2", "Second parent of Nested Subtopic 2.1 should be Subtopic 2")
    ...
    def test_nested_header_chunking(self):
        ...
        self.assertGreater(
            len(subtopic_2_1_chunks), 0, 
            "Should have chunks for Nested Subtopic 2.1"
        )
    ...
    def test_path_elements_structure(self):
        ...
        nested_chunks = [
            chunk for chunk in chunks 
            if chunk.metadata.get("nested_level", 0) > 0
        ]
        
        self.assertGreater(len(nested_chunks), 0, "Should have chunks with nested headers")
