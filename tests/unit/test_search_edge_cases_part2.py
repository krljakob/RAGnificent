...
class TestSearchEdgeCasesPart2(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.embedding_patcher = mock.patch(
            "RAGnificent.rag.search.get_embedding_model"
        )
        ...
        self.vector_store_patcher = mock.patch(
            "RAGnificent.rag.search.get_vector_store"
        )
    ...
    def test_embedding_error_handling(self):
        ...
        self.assertEqual(
            len(results), 0, "Should return empty results on embedding error"
        )
    ...
    def test_vector_store_error_handling(self):
        ...
        results = self.search.search("test query")
        self.assertEqual(
            len(results), 0, "Should return empty results on vector store error"
        )
    ...
    def test_empty_search_results(self):
        ...
        self.assertEqual(
            len(results),
            0,
            "Should return empty results when vector store returns none",
        )
    ...
    def test_malformed_search_results(self):
        ...
        self.assertEqual(
            results[0].content, "", "Should use empty string for missing content"
        )
        self.assertEqual(
            results[0].document_id, "", "Should use empty string for missing id"
        )
        self.assertEqual(
            results[2].document_id, "", "Should use empty string for missing id"
        )
        self.assertEqual(
            results[3].content, "", "Should use empty string for missing content"
        )
    ...
    def test_reranking_error_handling(self):
        ...
        with mock.patch.object(SemanticSearch, "_rerank_results") as mock_rerank:
            ...
            results = self.search.search("test query", rerank=True)
            self.assertEqual(
                len(results), 1, "Should return original results on reranking error"
            )
