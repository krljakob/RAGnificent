...
"""
        ...
        self.assertLessEqual(len(chunks), 5, "Should not create excessive chunks for simple content")
        ...
        self.assertEqual(len(chunks), expected_sections, f"Should create {expected_sections} chunks for {expected_sections} sections")

        # Verify chunk content contains expected headers
        chunk_contents = [chunk.content for chunk in chunks]
        self.assertIn("# Title", chunk_contents[0], "First chunk should contain main title")
        ...
        for header in section_headers:
            ...
            self.assertTrue(found_in_chunk, f"Header '{header}' should be found in at least one chunk")

        # Verify metadata structure and completeness
        for i, chunk in enumerate(chunks):
            self.assertIsNotNone(chunk.metadata, f"Chunk {i} should have metadata")
            self.assertIn("heading", chunk.metadata, f"Chunk {i} should have heading metadata")
            self.assertNotEqual(chunk.metadata["heading"], "", f"Chunk {i} heading should not be empty")
            self.assertEqual(chunk.source_url, self.test_url, f"Chunk {i} should have correct source URL")
            self.assertEqual(chunk.chunk_type, "section", f"Chunk {i} should be of type 'section'")
    ...
    def test_create_large_chunks(self):
        ...
        self.assertGreater(len(chunks), 1, "Large content should be split into multiple chunks")
        
        # Verify chunk size constraints are respected
        for i, chunk in enumerate(chunks):
            self.assertLessEqual(
                len(chunk.content), small_chunker.chunk_size + 200,  # Allow some tolerance
        ...
        for chunk in chunks:
            self.assertEqual(chunk.metadata["heading"], "# Large Section", 
                           "All chunks from same section should have same heading")
            self.assertEqual(chunk.source_url, self.test_url, 
                           "All chunks should have correct source URL")
        
        # Verify content continuity (no content is lost)
        combined_content = " ".join(chunk.content for chunk in chunks)
        self.assertIn("Large Section", combined_content, "Header should be preserved in chunks")
        ...
        self.assertGreater(word_count_chunks, word_count_original * 0.8, 
    ...
    def test_save_chunks_jsonl(self):
        ...
        with open(jsonl_path, "r") as f:
            ...
            for i, line in enumerate(lines):
                ...
                required_fields = ["id", "content", "metadata", "source_url", "chunk_type"]
                for field in required_fields:
                    self.assertIn(field, chunk_data, f"Chunk {i} missing required field: {field}")
                
                # Verify field values match original chunks
                original_chunk = chunks[i]
                self.assertEqual(chunk_data["id"], original_chunk.id, f"Chunk {i} ID mismatch")
                self.assertEqual(chunk_data["content"], original_chunk.content, f"Chunk {i} content mismatch")
                self.assertEqual(chunk_data["source_url"], original_chunk.source_url, f"Chunk {i} source_url mismatch")
                self.assertEqual(chunk_data["chunk_type"], original_chunk.chunk_type, f"Chunk {i} chunk_type mismatch")
    ...
    def test_create_semantic_chunks(self):
        ...
        self.assertGreater(len(markdown_chunks), 0, "Should create at least one chunk from markdown")
        self.assertIsNotNone(markdown_chunks[0].metadata, "Markdown chunk should have metadata")
        self.assertIn("heading", markdown_chunks[0].metadata, "Markdown chunk should have heading metadata")
        self.assertEqual(markdown_chunks[0].source_url, self.test_url, "Chunk should have correct source URL")
        ...
        self.assertEqual(len(text_chunks), 1, "Plain text should create exactly one chunk")
        self.assertEqual(text_chunks[0].chunk_type, "text_chunk", "Plain text should create text_chunk type")
        self.assertEqual(text_chunks[0].content, text_content, "Plain text content should be preserved exactly")
        self.assertEqual(text_chunks[0].source_url, self.test_url, "Plain text chunk should have correct source URL")
        ...
        self.assertEqual(len(whitespace_chunks), 0, "Whitespace-only content should create no chunks")
