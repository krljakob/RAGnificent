...
def fix_imports_in_file(file_path, is_test_file=False):
    ...
    if (
        is_test_file
        and "import RAGnificent" not in content
        and "from RAGnificent" not in content
    ):
        return 0
...
"""
    ...
    elif docstring_match := re.search(r'"""[^"]*"""', content):
    ...
    modified_content = re.sub(
        r"import\s+RAGnificent\.(\w+(?:\.\w+)*)", r"import \1", modified_content
    )
