...
def test_convert_html_to_markdown():
    ...
    assert len(header_lines) >= 2, "Should have at least 2 header lines (title + main title)"
...
* List item 2
    ...
    for i, chunk in enumerate(chunks):
        ...
        assert len(chunk.strip()) > 0, f"Chunk {i} should not be empty or whitespace-only"
    ...
    assert "## Section 1" in combined_content, "Section 1 header should be preserved in chunks"
    assert "## Section 2" in combined_content, "Section 2 header should be preserved in chunks"
    assert "This is a test paragraph." in combined_content, "Paragraph content should be preserved"
    ...
    for i, chunk in enumerate(chunks):
        assert len(chunk) <= chunk_size + 100, f"Chunk {i} size {len(chunk)} exceeds reasonable limit based on chunk_size {chunk_size}"
    ...
    preservation_ratio = len(preserved_words) / len(original_words) if original_words else 1
    assert preservation_ratio > 0.8, f"Should preserve most content, got {preservation_ratio:.2%} preservation"
...
def test_error_handling():
    ...
    assert exc_info.value is not None, "Should raise specific exception for malformed HTML"

    # Test chunking with invalid parameters
    with pytest.raises(Exception) as exc_info:
        ragnificent_rs.chunk_markdown("test", -1, -1)
    assert exc_info.value is not None, "Should raise specific exception for negative chunk parameters"
    ...
    try:
        ...
        assert isinstance(result, str), "Empty HTML should return empty string or valid result"
